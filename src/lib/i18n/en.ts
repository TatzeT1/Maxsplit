import type { Dictionary } from "@/lib/i18n/translate";

/**
 * English dictionary. Typed against the German dictionary's shape so a
 * missing or extra key is a compile error, not a silent fallback at runtime.
 */
export const en: Dictionary = {
  app: {
    name: "Split",
    tagline: "Split expenses without doing the math.",
  },
  common: {
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    close: "Close",
    back: "Back",
    next: "Next",
    confirm: "Confirm",
    loading: "Loading …",
    retry: "Try again",
    moreActions: "More actions",
    somethingWentWrong: "Something went wrong.",
    unsavedChanges: "You have unsaved changes.",
    offline: "No connection — changes aren't being saved right now.",
  },
  nav: {
    groups: "Groups",
    activity: "Activity",
    settings: "Settings",
    profile: "Profile",
    admin: "Admin",
    signOut: "Sign out",
  },
  profile: {
    title: "Profile",
    displayNameLabel: "Display name",
    displayNamePlaceholder: "e.g. Max",
    displayNameHint: "This name is shown in your groups.",
    emailLabel: "Email",
    save: "Save",
    saveSuccess: "Saved.",
    saveError: "Couldn't save it.",
    errorInvalidName: "Please enter a name.",
    paymentDetailsTitle: "Payment details",
    paypalEmailLabel: "PayPal email",
    paypalEmailPlaceholder: "you@example.com",
    paypalMeHandleLabel: "PayPal.Me link",
    paypalMeHandlePlaceholder: "e.g. https://paypal.me/maxrobin",
    ibanLabel: "IBAN",
    ibanPlaceholder: "e.g. DE89 3704 0044 0532 0130 00",
    paymentDetailsHint:
      "Shown to your group members so they can pay you back. Leave a field empty to hide it.",
    errorInvalidPaypalEmail: "Please enter a valid email address.",
    errorInvalidIban: "Please enter a valid IBAN.",
    errorInvalidPaypalMeHandle:
      'Please enter your full PayPal.Me link, e.g. "https://paypal.me/maxrobin" — just your username also works.',
    guideTitle: "How does this work?",
    guideIntro: "Three ways your roommates can pay you back — set up one or more of them.",
    guidePaypalMeTitle: "PayPal.Me link",
    guidePaypalMeBadge: "Fastest",
    guidePaypalMeSummary: "One tap, straight to the right person.",
    guidePaypalMeDetail:
      'Open your PayPal.Me profile (create a username at paypal.me if you don\'t already have one) and copy the full link. Paste it here exactly as copied — just your bare username works too. Members with an open balance then see a "Pay now" button that opens PayPal and suggests you as the recipient — the person paying still enters the amount themselves there.',
    guidePaypalMeExampleLink: "https://paypal.me/maxrobin",
    guidePaypalMeSetupLink: "Set up PayPal.Me",
    guidePaypalEmailTitle: "PayPal email",
    guidePaypalEmailSummary: "For a manual payment in the PayPal app.",
    guidePaypalEmailDetail:
      'Enter the email address you use to sign in to PayPal — you\'ll find it in your PayPal profile under "Account settings". Members copy it with a tap and paste it into PayPal as the recipient by hand.',
    guideIbanTitle: "IBAN",
    guideIbanSummary: "For a classic bank transfer.",
    guideIbanDetail:
      "Open your banking app, copy your IBAN, and paste it here. Members who owe you money only see it on their own screen and can copy it with a tap to paste into their own transfer.",
    onboardingReplayLink: "Watch the setup guide again",
  },
  onboarding: {
    skip: "Skip",
    stepLabel: "Step {{current}} of {{total}}",
    welcomeTitle: "Welcome to Split, {{name}}!",
    welcomeSubtitle: "A quick look at how it works — takes about a minute.",
    featureGroupsTitle: "Create or join a group",
    featureGroupsDescription:
      "Start a group or join one via a link — for a shared flat, a trip, or friends.",
    featureSplitTitle: "Split expenses",
    featureSplitDescription: "Add an expense — Split automatically works out who owes whom what.",
    featureBalanceTitle: "Settle up balances",
    featureBalanceDescription:
      "Keep track and settle open amounts directly via PayPal or bank transfer.",
    continueButton: "Continue",
    backButton: "Back",
    paymentTitle: "Set up payment details",
    paymentSubtitle:
      "So your roommates can pay you back easily. You can change this in your profile anytime.",
    paymentFinish: "Done",
    laterHint: 'No rush — you\'ll always find this under "Profile" later.',
  },
  theme: {
    light: "Light",
    dark: "Dark",
    system: "System",
    toggleLabel: "Toggle theme",
  },
  language: {
    toggleLabel: "Change language",
    de: "Deutsch",
    en: "English",
  },
  installApp: {
    button: "Install app",
    iosTitle: "Install the app",
    iosDescription: "Add Split to your home screen in a couple of taps.",
    iosStep1: "Tap Share at the bottom of Safari.",
    iosStep2: 'Choose "Add to Home Screen".',
  },
  auth: {
    signInTitle: "Welcome to Split",
    signInSubtitle: "Sign in to see your groups.",
    signInWithGoogle: "Sign in with Google",
    signInError: "Sign-in failed. Please try again.",
    signingIn: "Signing in …",
    orDivider: "or",
    continueWithEmail: "Continue with email",
    nameLabel: "Name",
    namePlaceholder: "e.g. Max",
    emailLabel: "Email",
    passwordLabel: "Password",
    signInButton: "Sign in",
    createAccount: "Create account",
    sendResetLink: "Send reset link",
    switchToSignUp: "New here? Create an account",
    switchToSignIn: "Already have an account? Sign in",
    forgotPassword: "Forgot password?",
    resetPasswordSent: "Password reset email sent — check your inbox.",
    errorEmailInUse: "An account with this email already exists.",
    errorWeakPassword: "Password should be at least 6 characters.",
    errorInvalidEmail: "That email address doesn't look right.",
    errorInvalidCredentials: "Incorrect email or password.",
    errorTooManyRequests: "Too many attempts. Please wait a bit and try again.",
    errorPopupClosed: "Sign-in was cancelled.",
    heroTitle: "Split expenses",
    heroTitleLine2Prefix: "without",
    heroTitleHighlight: "doing the math",
    heroDescription:
      "Track shared costs with roommates, trips, and friends — split fairly and settle up in seconds.",
    heroSocialProof: "No ads. No fees. Just fair splits.",
    heroImageAltGroceries: "Two roommates unpacking groceries together in their kitchen",
    heroImageAltBill: "A group of friends laughing over the bill at a café",
    heroImageAltTravel: "Three friends checking their phone together on a road trip",
    featuresEyebrow: "Everything in one app",
    featuresTitle: "Built for every shared bill",
    featuresSubtitle: "Roommates, trips, or friends — Split has what you need.",
    featureInviteTitle: "Invite friends",
    featureInviteDescription:
      "Share a link — everyone joins the group in one click after signing in with Google.",
    featureSplitTitle: "Split flexibly",
    featureSplitDescription:
      "Equally, by shares, percent, or exact amount — with categories and multiple payers per expense.",
    featureBalanceTitle: "Balances at a glance",
    featureBalanceDescription:
      "Split automatically works out who owes whom, and suggests the fewest transfers needed to settle up.",
    featurePdfTitle: "Settlement as a PDF",
    featurePdfDescription:
      "Export the settlement as a PDF and share it via link — even with people who don't have a Split account.",
    featureFreeTitle: "100% free",
    featureFreeDescription:
      "No ads, no subscription trap, no hidden costs. Just fair splitting for everyone.",
  },
  groups: {
    title: "Your groups",
    empty: "You're not in any group yet. Create one or join one.",
    create: "Create group",
    createTitle: "New group",
    editGroup: "Edit group",
    editTitle: "Edit group",
    editError: "Couldn't update the group.",
    join: "Join group",
    joinTitle: "Join group",
    nameLabel: "Group name",
    namePlaceholder: "e.g. Shared flat",
    iconPickerLabel: "Choose an icon",
    iconReset: "Remove icon",
    currencyLabel: "Currency",
    membersLabel: "Members (optional)",
    membersHint:
      "Add names now, even without a Split account — once they join, they just pick their name.",
    addAnotherMember: "Add another person",
    inviteCodeLabel: "Invite code",
    inviteCodePlaceholder: "e.g. AB3F9K7Q",
    inviteCodeHint: "Ask a group member for the code.",
    inviteCodeCopied: "Invite link copied.",
    shareInvite: "Share invite link",
    members: "Members",
    membersCount: "{{count}} members",
    memberCountSingular: "1 member",
    memberSince: "Member since {{date}}",
    createdOn: "Since {{date}}",
    balanceSettled: "Settled up",
    balanceOwedToYouLabel: "You're owed",
    balanceYouOweLabel: "You owe",
    moreMembers: "+{{count}}",
    roleOwner: "Creator",
    roleAdmin: "Admin",
    roleMember: "Member",
    makeAdmin: "Make admin",
    removeAdmin: "Remove admin rights",
    removeMember: "Remove",
    removeMemberConfirm: "Remove {{name}} from the group?",
    removeMemberConfirmBody: "{{name}} will lose access to this group.",
    removeMemberError: "Removing failed.",
    unsettledBalanceError: "There's still an open balance. Settle up first, then try again.",
    roleChangeError: "Couldn't change the role.",
    leaveGroup: "Leave group",
    leaveGroupConfirm: "Really leave this group?",
    leaveGroupConfirmBody: "You'll lose access to this group.",
    leaveGroupError: "Leaving failed.",
    ownerCannotLeave: "As the creator, you can't leave the group. Delete it instead.",
    archiveGroup: "Archive group",
    deleteGroup: "Delete group",
    deleteGroupConfirm: "Really delete this group?",
    deleteGroupConfirmBody: "All expenses and payments will be permanently deleted.",
    deleteGroupError: "Deleting failed.",
    createError: "Couldn't create the group.",
    joinError: "Couldn't join. Check the code.",
    joinSuccess: "You're now a member of {{name}}.",
    addPlaceholder: "Add person",
    addPlaceholderTitle: "Add person",
    addPlaceholderHint:
      "For people who aren't on Split yet. You can include them in expenses right away — once they join, they just pick their name and everything gets linked to them.",
    placeholderNameLabel: "Name",
    placeholderNamePlaceholder: "e.g. Max",
    addPlaceholderError: "Couldn't add them.",
    renamePlaceholder: "Rename",
    renamePlaceholderError: "Couldn't rename them.",
    notJoinedBadge: "Not joined yet",
    joinStep2Title: "Are you already on the list?",
    joinStep2Hint: "Pick your name so past expenses get linked to you — or join as a new person.",
    joinAsNew: "I'm new",
    joinContinue: "Continue",
    selfSuffix: " (you)",
    copyPaypal: "PayPal",
    copyIban: "IBAN",
  },
  expenses: {
    title: "Expenses",
    empty: "No expenses yet. Add the first one.",
    add: "Add expense",
    addTitle: "New expense",
    editTitle: "Edit expense",
    duplicate: "Duplicate",
    deleteConfirm: "Really delete this expense?",
    deleteConfirmBody: "This can't be undone.",
    descriptionLabel: "Description",
    descriptionPlaceholder: "e.g. Weekly groceries",
    amountLabel: "Amount",
    dateLabel: "Date",
    paidByLabel: "Paid by",
    paidByOne: "{{name}} paid",
    paidByMultiple: "{{names}} paid",
    multiplePayers: "Multiple people paid",
    payerAmountsHint: "Enter how much each person paid.",
    gamePickerTitle: "How should we decide?",
    gamePickerIntro: "Pick a game to decide who pays.",
    gameCategoryLuckTitle: "Games of chance",
    gameCategoryLuckBlurb: "Pure luck decides who pays.",
    gameCategorySkillTitle: "Minigames",
    gameCategorySkillBlurb: "Skill, not luck — always head-to-head.",
    gamePreviewHowTitle: "How it works",
    gamePreviewStart: "Let's go",
    gamePreviewBack: "Back",
    gameLadderTitle: "More than two playing?",
    gameLadderExplainer:
      "Then it runs as a knockout ladder: whoever loses a duel pays and is out. Whoever wins stays on and faces the next drawn challenger — until enough payers are decided.",
    gameNameLottery: "Who pays?",
    gameBlurbLottery: "Tap faces until one catches someone.",
    gameNameWheel: "Wheel of fortune",
    gameBlurbWheel: "Spin it and see where the needle lands.",
    gameNameSlot: "Slot machine",
    gameBlurbSlot: "Pick a stake, pull, and hope.",
    gameNameScratch: "Scratch cards",
    gameBlurbScratch: "Scratch until the result shows.",
    gameNameTicTacToe: "Tic-Tac-Toe",
    gameBlurbTicTacToe: "Three in a row wins.",
    gameNameConnectFour: "Connect Four",
    gameBlurbConnectFour: "Drop discs, four in a row wins.",
    gameNameMemory: "Memory duel",
    gameBlurbMemory: "Find pairs — fewer pairs pays.",
    gameNameReaction: "Reaction duel",
    gameBlurbReaction: 'Wait for "Go!", tap faster.',
    gameHowLottery:
      "A grid of 16 to 32 faces appears. Everyone taps one in turn — whoever gets a laughing face pays. You decide up front how many laughing faces are hidden in the grid.",
    gameHowWheel:
      "Every person gets a slice of the wheel. You spin it as many times as people should pay — wherever the needle lands, that person pays and drops out of the wheel.",
    gameHowSlot:
      "Pick a stake and pull the lever. The reels land on someone, who owes that stake — again and again, until the whole bill is allocated. The same person can be hit more than once.",
    gameHowScratch:
      "Everyone scratches their own card. A few of them are blanks — whoever gets one pays.",
    gameHowTicTacToe:
      "Two people take turns placing ✕ and ◯ on a 3×3 grid. Three in a row wins, and whoever loses pays. A draw goes straight into overtime.",
    gameHowConnectFour:
      "Two people take turns dropping discs into one of seven columns. First to get four in a row — horizontal, vertical, or diagonal — wins. Whoever loses pays.",
    gameHowMemory:
      "18 cards lie face down. You take turns flipping two — find a pair and you keep going. Whoever has fewer pairs at the end pays.",
    gameHowReaction:
      'Put the phone between you and tap your half when you\'re ready. After a random pause, "Go!" appears — whoever taps first wins. Tapping too early loses.',
    duelCountHint: "That's how many duels get played — whoever loses one pays.",
    duelMatchEyebrow: "Duel {{number}} of {{total}}",
    duelVersus: "{{a}} vs. {{b}}",
    duelHandoffHint: "Hand the phone to {{a}} and {{b}}.",
    duelStarts: "{{name}} goes first.",
    duelHandoffStart: "Let's go",
    duelTurnEyebrow: "Up now",
    duelLostStamp: "Lost!",
    duelLostCaption: "Loses to {{name}} and pays.",
    duelDrawNotice: "Draw — go again!",
    duelProgress: "{{found}} of {{target}} payers decided",
    duelRestart: "Restart",
    ticTacToeTitle: "Tic-Tac-Toe",
    ticTacToeIntro: "Two players, alternating ✕ and ◯. Three in a row wins — whoever loses pays.",
    ticTacToeHint: "Tap an empty cell.",
    ticTacToeSuddenDeath: "Overtime: placing a fourth mark removes your oldest one.",
    ticTacToeCellLabel: "Row {{row}}, column {{col}}",
    ticTacToeCellLabelTaken: "Row {{row}}, column {{col}}, {{name}}",
    connectFourTitle: "Connect Four",
    connectFourIntro: "Two players drop discs — four in a row wins, whoever loses pays.",
    connectFourHint: "Tap a column to drop a disc.",
    connectFourColumnLabel: "Column {{n}}",
    connectFourColumnFull: "Column {{n}} is full",
    memoryTitle: "Memory duel",
    memoryIntro: "Two players flip pairs — whoever has fewer at the end pays.",
    memoryHint: "Flip two cards.",
    memoryPairFound: "Pair! {{name}} goes again.",
    memoryNoPair: "No pair — {{name}}'s turn now.",
    memoryScoreLabel: "{{name}}: {{count}} pairs",
    memoryCardHidden: "Face-down card",
    memoryCardRevealed: "Revealed card",
    reactionTitle: "Reaction duel",
    reactionIntro:
      'Put the phone between you, wait for "Go!", tap faster. Tapping too early loses.',
    reactionTapWhenReady: "Tap when you're ready",
    reactionReady: "Ready!",
    reactionWaitingFor: "Waiting for {{name}} …",
    reactionSteady: "Steady …",
    reactionGo: "Go!",
    reactionFalseStart: "False start!",
    reactionTimeMs: "{{ms}} ms",
    reactionTooClose: "Too close! Go again.",
    reactionFaster: "Faster!",
    gameStart: "Start game",
    gamePoolLabel: "Who's playing?",
    gamePoolMinHint: "At least two people have to play.",
    gameCountLabel: "How many pay?",
    gameCountDecrease: "Fewer",
    gameCountIncrease: "More",
    gameCaughtStamp: "Gotcha!",
    gameResultEyebrow: "Result",
    gameResultOne: "{{name}} pays.",
    gameResultMultiple: "{{names}} pay.",
    gameApply: "Apply",
    gamePlayAgain: "Reshuffle",
    lotteryTitle: "Who pays?",
    lotteryIntro:
      "Pass the phone around: a grid of 16 to 32 faces appears. Everyone taps one in turn. Whoever gets a laughing face pays.",
    lotteryCountHint: "That's how many laughing faces are hidden in the grid.",
    lotteryTurnEyebrow: "Now up",
    lotteryTurnLabel: "{{name}}'s turn",
    lotteryTapAnyHint: "Tap any face.",
    lotteryProgress: "{{found}} of {{target}} found",
    lotteryCardUntapped: "Tap this face",
    lotteryRevealPay: "Pays!",
    lotterySafe: "Safe!",
    lotteryOverviewTitle: "Who's gambled away how much?",
    lotteryOverviewEmpty: "No one's played 🎲 Who pays? yet.",
    lotteryRoundsLostOne: "1 round lost",
    lotteryRoundsLost: "{{count}} rounds lost",
    lotteryRoundsNone: "Nothing lost yet",
    wheelTitle: "Wheel of fortune",
    wheelIntro: "Pass the phone around: the wheel decides who pays.",
    wheelCountHint: "That's how many spins it takes before every payer is decided.",
    wheelSpin: "Spin",
    wheelSpinAgain: "Spin again",
    wheelSpinningLabel: "The wheel is spinning …",
    wheelRoundResult: "{{name}} pays this round.",
    wheelProgress: "{{found}} of {{target}} decided",
    slotTitle: "Slot machine",
    slotIntro:
      "Pass the phone around: pick a stake, pull the lever — whoever it lands on owes that stake. Again and again, until the whole bill is allocated.",
    slotNoAmountHint: "Enter an amount first, before you play.",
    slotStakeLabel: "Stake per spin",
    slotStakeHint: "Charged on every spin until the whole amount is allocated.",
    slotStakeClamped: "Capped at {{amount}} — that's all that's left.",
    slotRemainingLabel: "Still open",
    slotAllocatedLabel: "Allocated",
    slotFullyAllocated: "Fully allocated!",
    slotTallyLabel: "Allocated so far",
    slotTallyEmpty: "No one's lost yet.",
    slotPull: "Pull the lever",
    slotPullAgain: "Pull again",
    slotSpinningLabel: "The reels are spinning …",
    slotRoundResult: "{{name}} pays {{amount}} this round.",
    scratchTitle: "Scratch cards",
    scratchIntro: "Everyone scratches their own card. Whoever gets a blank pays.",
    scratchCountHint: "That's how many cards are blanks.",
    scratchCardRevealHint: "Scratch or tap to reveal.",
    scratchRevealButton: "Reveal",
    scratchResultPay: "You pay!",
    scratchResultSafe: "Lucky again!",
    scratchProgress: "{{found}} of {{target}} revealed",
    splitModeLabel: "Split",
    splitEqual: "Equal",
    splitShares: "Shares",
    splitPercent: "Percent",
    splitExact: "Exact amount",
    splitGame: "Game",
    splitEqualHint: "Split equally among all members.",
    splitSharesHint: "Split by shares, e.g. 2 shares for Anna, 1 for Ben.",
    splitPercentHint: "The percentages must add up to 100%.",
    splitExactHint: "Enter the exact amount for each person.",
    sharesUnit: "shares",
    remaining: "{{amount}} left",
    overBy: "{{amount}} too much",
    percentRemaining: "{{percent}}% left",
    percentOverBy: "{{percent}}% too much",
    categoryLabel: "Category",
    categoryPlaceholder: "Choose a category",
    emojiPickerLabel: "Choose an emoji",
    emojiReset: "Use category icon",
    filterAllCategories: "All categories",
    searchPlaceholder: "Search expenses …",
    noResults: "No expenses found.",
    saveError: "Couldn't save the expense.",
    amountMismatch: "The split doesn't add up to the total amount.",
    errorInvalidDescription: "Please enter a description.",
    errorInvalidAmount: "Please enter a valid amount.",
    errorInvalidPayer: "Choose at least one payer.",
    errorInvalidParticipants: "Choose at least one participant.",
    errorNotOwner: "You can only edit or delete your own expenses.",
    deleteError: "Couldn't delete the expense.",
    celebrateTitle: "Expense saved!",
    detailPaidBy: "Paid by",
    detailSplit: "Split among",
  },
  categories: {
    groceries: "Groceries",
    restaurant: "Restaurant",
    transport: "Transport",
    housing: "Rent & housing",
    utilities: "Utilities",
    entertainment: "Entertainment",
    travel: "Travel",
    shopping: "Shopping",
    health: "Health",
    other: "Other",
  },
  analytics: {
    title: "Spending overview",
    byCategory: "By category",
    byMember: "Paid vs. share",
    paid: "paid",
    share: "share",
  },
  balances: {
    title: "Balances",
    settledUp: "You're all settled up.",
    youOwe: "You owe {{name}} {{amount}}",
    owesYou: "{{name}} owes you {{amount}}",
    totalOwed: "You owe {{amount}} in total",
    totalOwedToYou: "You're owed {{amount}} in total",
    simplifyDebts: "Simplify debts",
    simplifyDebtsHint: "Combines payments so fewer transfers are needed.",
    simplifyPreview: "Transfers before / after",
    transferSuggestion: "{{from}} pays {{to}} {{amount}}",
    downloadPdf: "Download as PDF",
    downloadPdfPending: "Creating PDF …",
    downloadPdfError: "Couldn't create the PDF.",
    copyLink: "Copy link",
    linkCopied: "Link copied",
    shareLinkHint: "This link works without a Split account — share it with everyone involved.",
    payNow: "Pay now",
    markPaid: "Mark as paid",
    copyPaypalEmail: "Copy PayPal email",
    paypalEmailCopied: "PayPal email copied",
    noPaypalMeHint: "No PayPal.Me on file",
  },
  settlements: {
    record: "Record payment",
    recordTitle: "Record payment",
    editTitle: "Edit payment",
    fromLabel: "From",
    toLabel: "To",
    amountLabel: "Amount",
    dateLabel: "Date",
    noteLabel: "Note (optional)",
    notePlaceholder: "e.g. via PayPal",
    celebrateTitle: "Payment recorded!",
    saveError: "Couldn't save the payment.",
    deleteConfirm: "Really delete this payment?",
    deleteConfirmBody: "This can't be undone.",
    errorInvalidParties: "“From” and “To” can't be the same person.",
    errorInvalidAmount: "Please enter a valid amount.",
    errorNotOwner: "You can only edit or delete your own payments.",
    deleteError: "Couldn't delete the payment.",
  },
  recurring: {
    title: "Recurring expenses",
    // Sits directly under the heading above, which already supplies the noun.
    add: "Add",
    addTitle: "New recurring expense",
    frequencyLabel: "Repeats",
    frequencyWeekly: "Weekly",
    frequencyMonthly: "Monthly",
    startDateLabel: "First date",
    participantsLabel: "Participants",
    nextRun: "Next run: {{date}}",
    paused: "Paused",
    pause: "Pause",
    resume: "Resume",
    deleteConfirm: "Really delete this rule?",
    deleteConfirmBody: "Future expenses will no longer be created automatically.",
    empty: "No recurring expenses.",
    saveError: "Couldn't save it.",
    errorNotOwner: "You can only edit or delete your own rules.",
  },
  activity: {
    title: "Activity",
    empty: "Nothing's happened here yet.",
    expenseAdded: "{{name}} added “{{description}}”",
    expenseEdited: "{{name}} edited “{{description}}”",
    expenseDeleted: "{{name}} deleted “{{description}}”",
    settlementRecorded: "{{from}} paid {{to}} {{amount}}",
    settlementEdited: "{{name}} edited a payment: {{description}}",
    settlementDeleted: "{{name}} deleted a payment: {{description}}",
  },
  chat: {
    title: "Chat",
    empty: "No messages yet. Write the first one!",
    placeholder: "Write a message …",
    send: "Send",
    unreadCountSingular: "1 new message",
    unreadCount: "{{count}} new messages",
    noPreview: "No messages yet",
    deleteConfirm: "Really delete this message?",
    deleteConfirmBody: "This can't be undone.",
    deleteError: "Couldn't delete the message.",
    sendError: "Couldn't send the message.",
    errorTooLong: "Messages can be at most {{count}} characters long.",
  },
  invite: {
    title: "You're invited",
    subtitle: 'Sign in with Google to join "{{name}}".',
    notFoundTitle: "Invalid invite",
    notFoundBody: "This invite link is expired or incorrect.",
    joining: 'Joining "{{name}}" …',
    joinError: "Joining failed. Please try again.",
    goToGroups: "Go to your groups",
  },
  errors: {
    notFound: "Not found.",
    forbidden: "You don't have permission for that.",
    network: "No connection. Check your internet.",
    unknown: "Unknown error.",
    dataLoadFailed: "Couldn't load data. Please reload the page.",
    errorCode: "Error code: {{code}}",
    sessionExpired: "Your session expired. Redirecting you to sign in again …",
  },
  settlementPdf: {
    badge: "Settlement Receipt",
    documentTitle: "Settlement – {{group}}",
    generatedAt: "Generated on {{date}}",
    summaryExpenses: "Total expenses",
    summaryMembers: "Members",
    summaryTransfers: "Open transfers",
    transfersHeading: "Suggested settlement transfers",
    transfersEmpty: "All balances are settled — nobody needs to transfer anything.",
    transferLine: "{{from}} pays {{to}}: {{amount}}",
    breakdownHeading: "Calculation per person",
    colPerson: "Person",
    colPaid: "Paid",
    colShare: "Share",
    colSettlementsSent: "Payments sent",
    colSettlementsReceived: "Payments received",
    colBalance: "Balance",
    settlementsHeading: "Payments already recorded",
    settlementsEmpty: "No payments recorded.",
    colDate: "Date",
    colFrom: "From",
    colTo: "To",
    colAmount: "Amount",
    colNote: "Note",
    paypalLabel: "PayPal:",
    ibanLabel: "IBAN:",
    expensesHeading: "All expenses",
    expensesEmpty: "No expenses recorded.",
    colDescription: "Description",
    colPaidBy: "Paid by",
    colSplitAmong: "Split among",
    footer: "Generated with Split · {{url}}",
    footerTagline: "Thanks for keeping things fair.",
  },
};
